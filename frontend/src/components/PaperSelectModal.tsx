...
import { Paperclip, Search, FileText, Check } from "lucide-react";
import { Paper } from "../store/paperStore";

interface PaperSelectModalProps {
  isOpen: boolean;
  papers: Paper[];
...
export default function PaperSelectModal({
  isOpen,
  papers,
  selectedPapers,
  onCancel,
  onConfirm,
}: PaperSelectModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      setQuery("");
      setSelected(new Set(selectedPapers.map((p) => p.id)));
    }
  ...
  const handleConfirm = () => {
    const result = papers.filter((p) => selected.has(p.id));
    onConfirm(result);
  };
      ...
      <div className="fixed inset-0 z-[301] flex items-center justify-center">
        ...
        >
          ...
          <div className="flex items-center gap-3 px-6 pt-6 pb-4 shrink-0">
            ...
            <div>
              <h2 className="text-sm font-semibold text-slate-900">指定文献</h2>
              <p className="text-xs text-slate-400 mt-0.5">选择参与上下文分析的论文</p>
            </div>
          </div>
